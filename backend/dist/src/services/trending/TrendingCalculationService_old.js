...
export class TrendingCalculationService {
     ...
     */
    async calculateTrendingScores(calculationType = 'realtime') {
        ...
        for (const counter of rippleCounters) {
            ...
            const score = (counter.actions24h * 2) +
                (counter.actions1h * 5) +
                (counter.participantsTotal * 0.5) +
                counter.boost;
            scores.push(score);
            results.push({
                rippleId: counter.rippleId,
                score,
                participants: counter.participantsTotal,
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            let topTenDays = 0;
            if (isTopTen) {
                if (counter.topTenSince) {
                    const daysSince = Math.floor((Date.now() - counter.topTenSince.getTime()) / (24 * 60 * 60 * 1000));
                    topTenDays = daysSince + 1;
                }
                else {
                    topTenDays = 1;
                }
            ...
            // Update ripple counter with top ten status
            await prisma.rippleCounter.update({
                where: { id: counter.id },
                data: {
                    topTenSince: isTopTen ? (counter.topTenSince || new Date()) : null,
                }
            });
