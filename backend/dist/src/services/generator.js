...
async function chooseBucket(userId, rippleId, waveId, defaultBucket) {
    ...
    if (allowed.length === 0)
        throw new Error('No allowed buckets for wave');
...
function pickModifier(mods, key) {
...
function fillPattern(textPattern, opts) {
    const mods = opts.modifiersJson ?? {};
    // simple {token} replacement
    return textPattern.replace(/\{(\w+)\}/g, (_, token) => {
        if (token === 'audience') {
...
async function pickTemplate(waveId, bucket, userId, rippleId) {
    ...
    const excludeIds = new Set(recent.map(r => r.micro.templateId).filter(Boolean));
...
export async function generateMicroActionForPrimary(userId) {
    // find primary ripple
    ...
    if (!defaultBucket)
        throw new Error('No default bucket or wave bucket available');
    // choose bucket
